...
void update_weights() {
   ...
   memcpy(A,dw1,sizeof(float)*LIST_SIZE);
  memcpy(B,dw1,sizeof(float)*LIST_SIZE);
    ...
    size_t global_item_size[] = {980, 120}; // Process the entire lists
    size_t local_item_size[] =  {980, 1}; // Process in groups of 64
  ...
  /*for(int i=0;i<117600;i++)
    cout<<A[i]<<" "<<C[i]<<endl;*/
	memcpy(dense_w,C,sizeof(float)*LIST_SIZE);
	/*	for (int k=0; k<980; k++) {
...
int main() {
  ...
  ret = clBuildProgram(program, 1, &device_id, NULL, NULL, NULL);
    ...
    kernel = clCreateKernel(program, "mult", &ret);
