...
void update_weights() 
{
  __m256d dense_local1,dense_local2,dense_local3,dense_local4,dense_local5,dense_local6,dense_local7,db1_local, dense_b_local, eta_local, mul_local, temp_local;

  //dense_b[i] -= eta * db1[i];
    //a_start = db1;
    //b_start = learn;
    //c_strat = dense_b;
    for(int n=0; n < 120; n = n+4)
    {
        db1_local = _mm256_loadu_pd(&db1[n]); 
        eta_local = _mm256_loadu_pd(&eta[0]);
		dense_b_local = _mm256_loadu_pd(&dense_b[n]);
		mul_local = _mm256_mul_pd(db1_local, eta_local);
		temp_local = _mm256_sub_pd(dense_b_local, mul_local); 
        _mm256_storeu_pd(&dense_b[n], temp_local);
    }
