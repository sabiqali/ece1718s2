...
double transpose[120][980];    //transpose matrix
double temp_sum[4]={0,0,0,0};	//temporary array
...
void forward_pass(int img[][32]) {
	...
	for(int i=0; i<980; i++)
	{
		for(int j=0; j<120; j++  )
		{
			transpose[j][i]=dense_w[i][j];
		}
	}

	// Dense Layer
	for (int i=0; i<120; i++) {
		__m256d transpose_local, dense_input_local, temp_sum_local, temp_sum1_local;
		dense_sum[i] = 0;
		dense_sigmoid[i] = 0;
		for (int j=0; j<980; j+=4) {  
			transpose_local=_mm256_loadu_pd(&transpose[i][j]);    //load from transpose matrix
			dense_input_local=_mm256_loadu_pd(&dense_input[j]);	  //load from dense_input

			temp_sum_local=_mm256_mul_pd(transpose_local,dense_input_local);    //Multiply the locals

			temp_sum1_local=_mm256_add_pd(_mm256_loadu_pd(&temp_sum[0]),temp_sum_local); //add the multiplication to the four values of temp_sum

			_mm256_storeu_pd(&temp_sum[0],temp_sum1_local);		//store the new values in temp_sum

			dense_sum[i]=temp_sum[0]+temp_sum[1]+temp_sum[2]+temp_sum[3];	//accumulate into dense_sum
...
void update_weights() {
  __m256d dense_local1,dense_local2,dense_local3,dense_local4,dense_local5,dense_local6,dense_local7, dense_local8, dense_local9;
	for (int i=0; i<120; i++) {
    ...
    //cout<<"testing11"<<j<<i<<endl;
		for (k=0; k<980; k+=4) {
			dense_local8 = _mm256_mul_pd(dense_local7,_mm256_loadu_pd(&dw1[k][i]));
			dense_local9 = _mm256_sub_pd(_mm256_loadu_pd(&dense_w[k][i]),dense_local8);
			_mm256_storeu_pd(&dense_w[k][i],dense_local9);
...
void backward_pass(double *y_hat, int *y, int img[][32]) {
	double delta4[10];
	//__m256d delta_local1, delta_local2;

	for (int i=0; i<7; i+=4) {
		//delta_local1 = _mm256_sub_pd(_mm256_loadu_pd(&y_hat[i]),_mm256_loadu_pd(&y[i]));
		//_mm256_storeu_pd(&delta4[i],delta_local1);
		//_mm256_storeu_pd(&db2[i],delta_local1);
	...
	for (int i=0; i<120; i++) {
		for (int j=0; j<7; j+=4) {
			//delta_local2 = _mm256_mul_pd(_mm256_loadu_pd(&dense_sigmoid[i]),_mm256_loadu_pd(&delta4[j]));
			//_mm256_storeu_pd(&dw2[i][j],delta_local2);
...
int main() {
	...
	/*for(int i=0; i<5;i++)
	{
		cout<<"dense_input "<<dense_input[i]<<endl;
	}

	/*double test[4];
	__m256d transpose_local;
	_mm256_storeu_pd(&test[0],transpose_local);

	for(int i=0; i<4;i++)
	{
		cout<<"dense_input "<<test[i]<<endl;
	} 

	int i=1;
	for(int j=0; j<4; j++)
	{
		cout<<"dense_w "<<transpose[i][j]<<endl;
	} */
