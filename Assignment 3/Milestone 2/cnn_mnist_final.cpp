...
void forward_pass(int img[][32]) {
  __m256d fpass1,fpass2,fpass3,fpass4,fpass5;
  ...
  double* temp_max = max_layer[0][0];
  double* temp_dense = dense_input;
  for(int i = 0;i<980;i+=4) {
     _mm256_storeu_pd(temp_dense,_mm256_loadu_pd(temp_max));
    temp_dense +=4;
    temp_max +=4;
  }
	/*int k=0;
  ...
  double temp[4];
	...
	for (int i=0; i<120; i++) {
       ...
       //double temp[] = {dense_w[j][i],dense_w[j+1][i],dense_w[j+2][i],dense_w[j+3][i]};
       fpass1 = _mm256_loadu_pd(&dense_w[j][i]);
       fpass2 = _mm256_loadu_pd(&dense_input[j]);
       fpass3 = _mm256_mul_pd(fpass1,fpass2);
       _mm256_storeu_pd(&temp[0],fpass3);
       dense_sum[i] += temp[0]+temp[1]+temp[2]+temp[3];
       //fpass4 = _mm256_loadu_pd(&dense_sum[i]);
       //fpass5 = _mm256_add_pd(fpass4,fpass3);
       //double temp = hsum_double_avx(fpass3);
       //dense_sum[i] += hsum_double_avx(fpass3);
       //_mm256_storeu_pd(&dense_sum[i],fpass5);
			//dense_sum[i] += dense_w[j][i] * dense_input[j];
		}
   /*if(i < 117) {
    dense_sum[i+1] -= dense_sum[i];
    dense_sum[i+2] -= dense_sum[i];
    dense_sum[i+3] -= dense_sum[i];
    }
    if(i == 117) {
    dense_sum[i+1] -= dense_sum[i];
    dense_sum[i+2] -= dense_sum[i];
	...
	for (int i=0; i<10; i++) {
		dense_sum2[i]=0;
		for (int j=0; j<120; j++) {
     //fpass1 = _mm256_loadu_pd(&dense_w2[j][i]);
       //fpass2 = _mm256_loadu_pd(&dense_sigmoid[j]);
       //fpass3 = _mm256_mul_pd(fpass1,fpass2);
       //_mm256_storeu_pd(&temp[0],fpass3);
       //dense_sum2[i] += temp[0]+temp[1]+temp[2]+temp[3];
...
void backward_pass(double *y_hat, int *y, int img[][32]) {
 ...
 //double temp[4];
  double delta4[10];
    ...
    _mm256_storeu_pd(&db1[i],_mm256_loadu_pd(&delta3[i]));
 ...
 double temp[4];
	for (int i=0; i<980; i++) {
      ...
      //double temp[4] = {dense_input[i],dense_input[i],dense_input[i],dense_input[i]};
      //temp[0] = dense_input[i];
      //temp[1] = dense_input[i];
      //temp[2] = dense_input[i];
      //temp[3] = dense_input[i];
      //bpass1 = _mm256_loadu_pd(&delta3[j]);
      //bpass2 = _mm256_loadu_pd(temp);
      //bpass3 = _mm256_mul_pd(bpass2,bpass1);
      //_mm256_storeu_pd(&dw1[i][j],bpass3);
  ...
  double *temp_db = db_conv[0][0];
  double *temp_dw = dw_max[0][0];
  for(int i=0;i<3920;i+=4) {
    _mm256_storeu_pd(temp_db,_mm256_loadu_pd(temp_dw));
    temp_db +=4;
    temp_dw +=4;
  }
	/*for (int filter_dim=0; filter_dim<5; filter_dim++) {
