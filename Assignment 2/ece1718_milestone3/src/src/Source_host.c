// Source file is "L/ece1718_milestone3/src/Source_host.cal"
...
////////////////////////////////////////////////////////////////////////////////
// Output FIFOs
extern fifo_u8_t *Source_host_Stream;
...
#define tokens_Stream Source_host_Stream->contents
...
static void write_Stream() {
	index_Stream = Source_host_Stream->write_ind;
}

static void write_end_Stream() {
	Source_host_Stream->write_ind = index_Stream;
}
...
void Source_host_initialize(schedinfo_t *si) {
	int i = 0;
	write_Stream();
...
void Source_host_scheduler(schedinfo_t *si) {
	int i = 0;
	...
	case my_state_SendLastData:
		goto l_SendLastData;
	default:
		printf("unknown state in Source_host.c : %s\n", stateNames[_FSM_state]);
		exit(1);
	}
...
l_SendData:
	if (isSchedulable_sendData_launch()) {
		int stop = 0;
		if (1 > SIZE_Stream - index_Stream + Source_host_Stream->read_inds[0]) {
			stop = 1;
		}
		if (stop != 0) {
